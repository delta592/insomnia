...
const shouldReturnSuccessCode = [
  ...
  '$PWD/packages/insomnia-inso/bin/inso run test -w packages/insomnia-inso/src/db/fixtures/nedb -e env_0568bc9 uts_a29c6e -f $PWD/packages',
...
describe('inso dev bundle', () => {
  ...
  describe('response and timeline has scripting effects', () => {
    ...
    it('send request with client cert and key', async () => {
      const input = `$PWD/packages/insomnia-inso/bin/inso run collection -w packages/insomnia-inso/src/db/fixtures/nedb --requestNamePattern "withCertAndCA" --verbose wrk_0b96eff -f $PWD/packages`;
      const result = await runCliFromRoot(input);
      if (result.code !== 0) {
        console.log(result);
      }
